...
class Product {
    ...
    public function deleteProduct($all_SKU) {
        $this->deletedProducts = $all_SKU;

        //create function that goes through array that contains SKUs of all the products to be deleted
        foreach($this->deletedProducts as $product) {

            //create the sql statement for deleting the checked product and initiate it
            $sql = "UPDATE products SET is_deleted = 1 WHERE SKU = '$product'";
            $stmt = $this->db_connection->stmt_init();
